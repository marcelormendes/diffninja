...
export function renderReview(report: ReviewReport): string {
  ...
  return [
    ...
    `<title>${escapeHtml(`Prismr review: ${report.title || "untitled diff"}`)}</title>`,
    `<style>${STYLES}</style>`,
    "</head>",
    "<body>",
    '<div class="wrap">',
...
function renderHeader(
  ...
  return [
    ...
    '<p class="brand"><span class="brand-mark" aria-hidden="true"></span>Prismr</p>',
...
function renderMockBanner(): string {
  return [
    ...
    "<p><strong>No API call was made.</strong> Prismr produced every judgment below from local placeholder logic so it can run without an API key. These scores are not an assessment of the change.</p>",
    "</div>",
  ].join("\n");
}
...
function renderScopeNote(): string {
  return [
    ...
    '<p>Prismr ranks hunks so a reviewer knows where to start. It does not approve, block or merge anything. A low or auto-passed rank means no signal was found, not that the code is correct. A human still owns the decision.</p>',
    "</div>",
  ].join("\n");
}
...
function renderFooter(report: ReviewReport): string {
  return [
    ...
    `<p>Generated by Prismr at ${escapeHtml(report.createdAt)} in ${escapeHtml(report.mode)} mode using ${formatInteger(report.modelCalls)} API calls.</p>`,
